...
class Level3: SKScene,SKPhysicsContactDelegate  {
    override func didMoveToView(view: SKView) {
        ...
        let bgImage = SKSpriteNode(imageNamed: "bg03")
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        if score == 25 {    // Level is complete
            self.removeAllChildren()
            var myTransition = SKTransition.doorsOpenVerticalWithDuration(2)
            let scene = Level4(size:CGSize(width: 1152, height: 2048))
            score = 25
        ...
        if lives <= 0 {   // Player lost all lives. End Game
            self.removeAllChildren()
            var myTransition = SKTransition.doorsOpenVerticalWithDuration(2)
            let scene = EndScreen3(size:CGSize(width: 1152, height: 2048))
            self.view?.presentScene(scene)
            lives = 3
    ...
    func createBricksSecondRow(){
        ...
        for counter in 1...5{
            ...
            let DownAction = SKAction.moveToY(CGFloat(size.height * 0.85), duration:1)
            let action = SKAction.rotateByAngle(CGFloat(M_PI), duration:2)
            brick.runAction(SKAction.repeatActionForever(action))
            brick.runAction(DownAction)
    ...
    func createBall(){
        ...
        ball.physicsBody?.restitution = 1.04
