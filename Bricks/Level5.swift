...
class Level5: SKScene,SKPhysicsContactDelegate  {
    override func didMoveToView(view: SKView) {
        ...
        var bgImage = SKSpriteNode(imageNamed: "bg05")
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        if score == 45 {    // Level is complete
            self.removeAllChildren()
            var myTransition = SKTransition.doorsOpenVerticalWithDuration(2)
            let scene = YouWon(size:CGSize(width: 1152, height: 2048))
            score = 45
        ...
        if lives <= 0 {   // Player lost all lives. End Game
            self.removeAllChildren()
            var myTransition = SKTransition.doorsOpenVerticalWithDuration(2)
            let scene = EndScreen5(size:CGSize(width: 1152, height: 2048))
            self.view?.presentScene(scene)
            lives = 3
    ...
    func createBricksThirdRow(){
        ...
        for counter in 1...1{
            ...
            let MoveAction = SKAction.moveByX(CGFloat(-size.width), y: 0, duration: 2)
            let ResetAction = SKAction.moveByX(CGFloat(size.width), y: 0, duration: 2)
