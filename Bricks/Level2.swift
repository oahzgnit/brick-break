...
class Level2: SKScene,SKPhysicsContactDelegate  {
    override func didMoveToView(view: SKView) {
        ...
        var bgImage = SKSpriteNode(imageNamed: "bg02")
...
//        score = 0
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        if score == 15 {    // Level is complete
            self.removeAllChildren()
            var myTransition = SKTransition.doorsOpenVerticalWithDuration(2)
            let scene = Level3(size:CGSize(width: 1152, height: 2048))
            score = 15
        ...
        if lives <= 0 {   // Player lost all lives. End Game
            self.removeAllChildren()
            var myTransition = SKTransition.doorsOpenVerticalWithDuration(2)
            let scene = EndScreen2(size:CGSize(width: 1152, height: 2048))
            self.view?.presentScene(scene)
            lives = 3
    ...
    func createBricksSecondRow(){
        ...
        for counter in 1...5{
            ...
            //brick.runAction(SKAction.repeatActionForever(action))
            let action = SKAction.moveToY(CGFloat(size.height * 0.85), duration:1)
