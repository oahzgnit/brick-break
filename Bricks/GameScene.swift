...
class GameScene: SKScene,SKPhysicsContactDelegate  {
    override func didMoveToView(view: SKView) {
        ...
        let bgImage = SKSpriteNode(imageNamed: "bg01")
        self.addChild(bgImage)
        bgImage.position = CGPointMake(self.size.width/2, self.size.height/2)
        //self.backgroundColor = UIColor(patternImage: UIImage(named: "ball")!)
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        if score == 5 {    // Level is complete
            self.removeAllChildren()
            var myTransition = SKTransition.crossFadeWithDuration(1)
            let scene = Level2(size:CGSize(width: 1152, height: 2048))  //iphone 5
            //score = 5
        ...
        if lives <= 0 {   // Player lost all lives. End Game
            ...
            let scene = LoseScreen(size:CGSize(width: 1152, height: 2048))
            self.view?.presentScene(scene)
            lives = 3
    ...
    func activeBall(){
        ...
        let myVector = CGVector(dx: 70, dy: 300)
